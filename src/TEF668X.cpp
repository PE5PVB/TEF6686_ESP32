...
void TEF6686::readRDS(byte showrdserrors) {
  if (millis() >= rdstimer + 43) {
    rdstimer += 43;
    uint16_t stat, a, b, c, d, err;
    devTEF_Radio_Get_RDS_Data(&stat, &a, &b, &c, &d, &err);
    ...
    if (bitRead(stat, 15) && !bitRead(stat, 13)) {                                                 // Data available, not a first-PI-only event
      rds.groupBuffer.set(RdsGroupBuffer::BLOCK_A, a, (err >> 14) & 0x03);
      rds.groupBuffer.set(RdsGroupBuffer::BLOCK_B, b, (err >> 12) & 0x03);
      rds.groupBuffer.set(RdsGroupBuffer::BLOCK_C, c, (err >> 10) & 0x03);
      rds.groupBuffer.set(RdsGroupBuffer::BLOCK_D, d, (err >> 8) & 0x03);
    }
  }

  uint16_t *groupData;
  uint8_t groupErr;
  if (!rds.groupBuffer.get(&groupData, &groupErr)) return;

  rds.rdsA = groupData[0];
  rds.rdsB = groupData[1];
  rds.rdsC = groupData[2];
  rds.rdsD = groupData[3];
  rds.rdsErr = ((uint16_t)groupErr) << 8;
  bitSet(rds.rdsStat, 9);
  bitSet(rds.rdsStat, 15);

  processRDSGroup(showrdserrors);
}
